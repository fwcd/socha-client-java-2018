...
public class IterativeDeepeningABPlayer extends TreeSearchPlayer {
	...
	/**
	 * Performs an iteratively deepening alpha-beta search. It ignores the provided
	 * level/depth and searches until the provided milliseconds have passed.
	 */
	@Override
	public double evaluate(GamePlay game, GameMove move, int[] role, int level, long ms) {
		...
		int depth = 0;
		try (ClosingExecutor threadPool = new ClosingExecutor()) {
