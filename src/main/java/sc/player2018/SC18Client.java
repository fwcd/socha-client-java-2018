...
public class SC18Client extends AbstractClient {
	private final String reservation;
	private final CopyableLogic logic;
	private boolean autoRelaunch = false;

	public SC18Client() throws IOException {
		this("localhost", SharedConfiguration.DEFAULT_PORT, "", "");
	}

	public SC18Client(String host, int port, String reservation, String strategy) throws IOException {
		// Launch client
		super(host, port);
	...
	public SC18Client(String host, int port, String reservation, CopyableLogic logic) throws IOException {
		// Launch client
		super(host, port);
		this.reservation = reservation;
		this.logic = logic;
	...
	public void relaunch() {
		new Thread(() -> {
			try {
				new SC18Client(getHost(), getPort(), reservation, logic);
			} catch (IOException e) {
