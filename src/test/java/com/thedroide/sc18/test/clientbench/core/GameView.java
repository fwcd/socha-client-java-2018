...
import com.thedroide.sc18.test.clientbench.utils.ColoredGrid;
...
public class GameView {
	...
	
	{
		...
		view.add(new JScrollPane(table), BorderLayout.EAST);
	}
	
	public GameView() {
		this(new HUIGameState(new GameState()));
	}
	
	public GameView(HUIGameState game) {
		update(game);
	}

	private void updateState(HUIGameState game) {
		grid.clear();
		
		Player red = game.getSCPlayer(HUIPlayerColor.RED);
		Player blue = game.getSCPlayer(HUIPlayerColor.BLUE);
	...
	public void update(HUIGameState gameState) {
