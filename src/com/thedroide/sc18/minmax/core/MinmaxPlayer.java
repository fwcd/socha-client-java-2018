...
public class MinmaxPlayer extends TemplatePlayer {
	...
	public boolean canPlayGame(GamePlay game) {
		if (game instanceof MinmaxGamePlay) {
			return true;
		} else {
			return false;
		}
	...
	public double heuristic(GamePlay game, GameMove move, int[] role) throws CannotPlayGameException, GameRuntimeException {
		try {
			return heuristic.heuristic((MinmaxGamePlay) game, (MinmaxMove) move, MinmaxEnumPlayer.of(role));
	...
	public boolean pruneMove(GamePlay game, GameMove move, int[] role) {
		try {
			return heuristic.pruneMove((MinmaxGamePlay) game, (MinmaxMove) move, MinmaxEnumPlayer.of(role));
		} catch (ClassCastException e) {
			throw new CannotPlayGameException(this, game, "Invalid game type.");
		}
	}
