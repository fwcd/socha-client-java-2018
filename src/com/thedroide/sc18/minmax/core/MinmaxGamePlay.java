 ...
 */
public class MinmaxGamePlay implements GamePlay, TreeNode {
	private static final long serialVersionUID = -6693551955267419333L;
	
	private GameState state;
	private List<MinmaxMove> moveHistory = new ArrayList<>();
	private List<MinmaxMove> legalMoves = null;
	 ...
	 */
	public MinmaxGamePlay() {
		
	}
	
	/**
	 * Constructs a new {@link MinmaxGamePlay} from the
	 * given {@link GameState} (Software Challenge API).
	 * 
	 * @param state - The GameState used as a base
	 */
	public MinmaxGamePlay(GameState state) {
		setState(state);
	}
	 ...
	 * move.
	 * 
	 * @return The next player as a {@link MinmaxEnumPlayer}.
	 */
	public MinmaxEnumPlayer nextHUIEnumPlayer() {
		return MinmaxEnumPlayer.of(state.getCurrentPlayerColor());
	}
	 ...
	 */
	public Field fieldOf(MinmaxEnumPlayer player) {
	...
	public int[] getWinner() {
		if (state.getRedPlayer().inGoal()) {
			return new int[] {MinmaxEnumPlayer.RED.getID()};
		} else if (state.getBluePlayer().inGoal()) {
			return new int[] {MinmaxEnumPlayer.BLUE.getID()};
		} else {
			return null;
		}
	}
	
	public synchronized void sortLegalMoves(Comparator<MinmaxMove> comparator) {
	...
	private synchronized void updateLegalMoves() {
		legalMoves = state
				.getPossibleMoves()
				.stream()
				.map(scMove -> new MinmaxMove(nextHUIEnumPlayer(), scMove))
	...
	public synchronized boolean makeMove(GameMove move) {
		try {
			MinmaxMove huiMove = (MinmaxMove) move;
	 ...
	 */
	public List<MinmaxMove> getLegalMovesList() {
	...
	@Override
	public MinmaxGamePlay spawnChild(GameMove move) throws GameRuntimeException {
		try {
			MinmaxGamePlay child = clone();
			child.makeMove(move);
			return child;
		} catch (CloneNotSupportedException e) {
			GUILogger.printStack(e);
	...
	@Override
	public MinmaxGamePlay clone() throws CloneNotSupportedException {
		MinmaxGamePlay clone = (MinmaxGamePlay) super.clone();
	 ...
	 */
	protected Player getSCPlayer(MinmaxEnumPlayer player) {
		switch (player) {
	...
	public String getNodeDescription() {
		return "R" + Integer.toString(getSCPlayer(MinmaxEnumPlayer.RED).getFieldIndex())
				+ "|B" + Integer.toString(getSCPlayer(MinmaxEnumPlayer.BLUE).getFieldIndex());
	}
