...
public class SmartHeuristic implements LeafHeuristic {
	...
	public double heuristic(HUIGamePlay gameBeforeMove, HUIMove move, HUIEnumPlayer player) {
		...
		for (Action action : move.getSCMove().getActions()) {
			if (action instanceof ExchangeCarrots) {
				...
				if (playerBeforeMove.getFieldIndex() > lastSaladField
						&& playerBeforeMove.getSalads() > 0) {
					// If the player is near the goal and needs to
					// drop salads, falling back is a good idea
					multiplier = 2;
				} else if (playerBeforeMove.getFieldIndex() > lastSaladField
		...
		double rating = (saladRating + fieldRating + carrotRating) * multiplier;
